...
export const GET = auth(async (req) => {
  ...
  try {
    ...
    // 1. Get Opening Balance (Balance after the last transaction before start date)
    const lastTransactionBefore = await prisma.transactionHistory.findFirst({
      where: {
        organizationId: orgId,
        impactedAccount: AccountType.CASH,
        date: { lt: start },
      },
      orderBy: { date: "desc" },
    });

    // If no transactions before, fallback to Organization opening balance
    ...
    if (lastTransactionBefore) {
      ...
      openingBalance = Number(org.openingCashBalance);
    }
    ...
    const transactions = await prisma.transactionHistory.findMany({
      ...
      orderBy: { date: "asc" }, // Ascending for ledger view
    });
